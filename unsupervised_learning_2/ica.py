...
class ICARunner:
    ...
    def get_num_components(self):
        ...
        if self.should_plot:
            ...
            self._plot_distribution(ica, num_components)
    ...
    def _plot_distribution(self, ica, num_components):
        plt.rcParams["figure.figsize"] = (12, 6)
        fig, ax = plt.subplots()
        xi = np.arange(1, ica.components_.shape[1] + 1, step=1)

        legend = []
        for i in range(num_components):
            legend.append(str(i + 1))
            plt.bar(xi, ica.components_[i])

        plt.legend(legend, loc="upper left")

        plt.xlabel("Features")
        plt.xticks(
            np.arange(0, ica.components_.shape[1] + 1, step=1)
        )  # change from 0-based array index to 1-based human-readable label
        plt.ylabel("Feature Weight")
        plt.title(f"{self.title} Distribution")
...
if __name__ == "__main__":
    ...
    if AUSTRALIA:
        ...
        aus_tester = ICARunner(
            aus_X_train,
            aus_y_train,
            title="Australian_Weather_ICA_NN",
            seed=42,
            plot=True,
        )
        aus_n_components = aus_tester.get_num_components()
