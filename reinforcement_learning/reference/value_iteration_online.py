            ...
            for action in range(env.nA):
                ...
                for i in range(len(env.P[state][action])):
                    ...
                    state_action_value = prob * (
                        reward + lmbda * stateValue[next_state]
                    )
                ...
                newStateValue[state] = action_values[
                    best_action
                ]  # update the value of the state
        if i > 1000:
            if (
                sum(stateValue) - sum(newStateValue) < 1e-04
...
def get_score(env, policy, episodes=1000):
    ...
    print("----------------------------------------------")
    print(
        "You took an average of {:.0f} steps to get the frisbee".format(
            np.mean(steps_list)
        )
    )
    print(
        "And you fell in the hole {:.2f} % of the times".format(
            (misses / episodes) * 100
        )
    )
    print("----------------------------------------------")
