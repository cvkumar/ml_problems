...
class ExpectationMaximizationTestCluster:
    ...
    def run(self):
        ...
        for k in self.clusters:
            ...
            if k == self.targetcluster and self.stats:
                ...
                pd_data.to_csv(
                    "cluster_em.csv", index=False, index_label=False, header=False
                )
        ...
        if self.gen_plot:
            # self.visualize()
            self.plot(
                ll, homogeneity_scores, completeness_scores, rand_scores, bic, aic
            )
    ...
    def visualize(self):
        ...
        for clust in range(0, self.targetcluster):
            ...
            plt.scatter(
                malignant[:, 0], malignant[:, 1], color=colors[clust], marker="x"
            )
        ...
        plt.title("Australian Weather Clustering")
